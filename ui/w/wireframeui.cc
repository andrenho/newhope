      ...
      rx(0), ry(0)
{
    ...
    WireframeUI::Speech(class Person const& person, std::string message) const
    {
        ...
        //int lines = ui_wrap_text(text, (win_w-50) / advance); // TODO
        int lines = 1;
        
        SDL_Rect r = { 0, win_h - (lines * TTF_FontLineSkip(main_font)) - 50, 
            win_w, (lines * TTF_FontLineSkip(main_font)) + 50 };
        SDL_SetRenderDrawColor(ren, 0, 0, 0, 255);
        SDL_RenderFillRect(ren, &r);
        int y = win_h - (lines * TTF_FontLineSkip(main_font)) - 25;
        
        char *token = strdup(message.c_str());
#ifdef __MINGW32__
...
#endif
		SDL_Surface* sf = TTF_RenderUTF8_Solid(main_font, token, 
